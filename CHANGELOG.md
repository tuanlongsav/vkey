...
hoặc các app Electron khác.** Hai thay đổi nhỏ ở đường nóng, không đổi cách gõ
khi ô nhập ổn định.

### ⌨️ Tầng gửi phím & Accessibility

- **Giữ trục NFD suốt một từ trong web content Electron** — khi Accessibility
  hiccup đổi phân loại ô giữa chừng (`.webContent` ↔ `.windowField`), vkey không
  còn lật sang đếm NFC rồi gửi thiếu backspace, ăn ngược vào chữ đã gõ. Chỉ
  chốt sau lần gửi đầu có backspace thật trên nhánh NFD và chỉ khi AX đã thấy
  `.webContent`; không chốt NFC (hộp thoại Lưu AppKit vẫn tự hội tụ) và không
  chốt trên `.unknown` (tránh đóng băng sai khi Save panel vừa mở).
- **Hết race timeout Accessibility giữa tap thread và refresh nền** — leo cây
  phân loại field dùng timeout theo từng element thay vì ghi đè timeout toàn
  tiến trình trong ~50 message mỗi phím.

### 🧪 Test

- 424 test pass (+3 cho chốt NFD và ngân sách AX tách đường nóng/nền).
...
- **Telegram: hết mất chữ ĐẦU** — gõ "gửi" nay ra đúng "gửi" (trước bị rớt chữ đầu thành "ửi"). Nguyên nhân: ô soạn tin Telegram xử lý chuỗi xoá + gõ lại bất đồng bộ ở chế độ gửi mặc định → rớt ký tự đầu khi thay cả cụm. Đã định tuyến Telegram sang gửi từng phím (`stepByStep`) để đồng bộ với text engine của app.
- **Telegram & ChatGPT: hết mất chữ GIỮA** — gõ "điều" nay ra đúng "điều" (trước bị "đều"). Whitelist 2 app native này dùng cách xoá ký tự theo grapheme (NFC), tránh xoá thừa 1 ký tự ở cụm nguyên âm mở (iêu/iểu…).

### 🧹 Kỹ thuật

- Thêm test regression (`testTelegramUsesStepByStep`, `testDieuTelegramFix`, `testTelegramChatGPTRouteThroughNFC`). 308/308 test pass.
- Cập nhật cert phát hành: cert Developer ID Application cũ (mất khỏi keychain sau lần đăng xuất Xcode) được thay bằng cert mới **cùng team U4B264GM2B** — designated requirement không đổi nên **không cần cấp lại quyền Trợ năng** khi cập nhật.

## [4.8] - 2026-07-01 — "Nâng cấp giao diện: token, tương phản & tương tác"

**Tinh chỉnh hệ thống thiết kế theo bản design v4.8 — dễ đọc hơn, phản hồi tương tác rõ hơn.**

### 🎨 Giao diện

- **Tương phản đạt chuẩn AA** — chữ phụ (hint/muted) và chữ trên badge được nâng độ tương phản (WCAG AA) ở cả 3 theme, dễ đọc hơn.
- **Màu nhấn có chiều sâu** — mỗi màu nhấn (Đỏ Saigon/Vàng/Xanh lá/Xanh dương/Tím) nay có đủ dải đậm/nhạt; nút bấm đổi màu khi rê chuột (đậm hơn) và nhấn giữ (đậm nhất) — trước đây không phản hồi.
- **Phản hồi tương tác & bàn phím** — nút, ô chọn, thẻ theme, ô màu nhấn, mục điều hướng và menu có **hiệu ứng rê chuột + viền focus** (điều hướng bằng bàn phím thấy rõ), tất cả **tôn trọng "Giảm chuyển động"** của hệ thống.
- **Bo góc đồng bộ theo theme** — tuỳ chọn "Sắc/Vừa/Tròn" nay ảnh hưởng cả menu, keycap, ô icon (trước đây chỉ một phần).
- **Trạng thái rỗng thân thiện** — tab Macro và Smart Switch khi chưa có dữ liệu hiển thị hướng dẫn + nút hành động rõ ràng.

### 🧹 Kỹ thuật

- Chuẩn hoá token (dải màu nhấn 5 mức, token chữ semantic AA, tracking/leading, `fgMuted`); gỡ file thừa `GlassTile`. 305/305 test pass.
...
- **Mật khẩu không lọt lịch sử clipboard** — mục do trình quản lý mật khẩu (1Password/Bitwarden/Keychain…) đánh dấu bí mật (`org.nspasteboard.ConcealedType` / Transient / AutoGenerated) không còn được lưu vào lịch sử clipboard kèm preview.
- **Loại dữ liệu đã gõ khỏi sao lưu** — thư mục Thống kê và n-gram (từ/cụm đã gõ, dạng plaintext) được đánh dấu loại khỏi iCloud/Time Machine.
- **Học n-gram tôn trọng quyền riêng tư** — không ghi n-gram khi tắt Thống kê, hoặc khi đang ở ô nhập bảo mật (secure input).
- **File tín hiệu Smart Switch an toàn hơn** — `/tmp/vkey_switch_<uid>` mở bằng `O_NOFOLLOW` + kiểm tra chủ sở hữu, chống tấn công chuyển hướng qua symlink.
- **Gói cập nhật từ điển được gia cố** — giới hạn kích thước tải về (25 MB), kiểm tra giới hạn cấu trúc, và cơ chế xác minh chữ ký Ed25519 (bao trùm toàn bộ dữ liệu ảnh hưởng hành vi, kiểm tra ở cả lúc tải lẫn lúc nạp file đã lưu) — sẵn sàng bật khi publisher ký server-side.

### 🐛 Sửa lỗi gõ

- **Auto dấu ă cho "a…k" chỉ áp cho địa danh** — "Đắk", "Lắk" vẫn tự thêm ă; nhưng "AK-47", "tak", "mak", "nak", "flak"… không còn bị thêm ă sai.
- **Không đảo "ao"→"oa" qua ranh giới âm tiết** — gõ "DaoTao", "BaoCao" (camelCase) giữ nguyên, không còn thành "DoaTao".

### 🧯 Ổn định dữ liệu

- **Nhập dữ liệu an toàn hơn** — file có khối thống kê hỏng nay báo "Lỗi nhập" thay vì báo thành công rồi âm thầm mất toàn bộ lịch sử.

### ⚡ Hiệu năng

- **Gợi ý từ nhanh hơn** — cache snapshot từ điển, không dựng lại mảng ứng viên mỗi lần gõ.

### 🧪 Kỹ thuật

- 303/303 test pass. Gộp logic lọc "Top từ" trùng lặp giữa 2 màn Thống kê về một nơi.
...
- **Viết hoa đầu câu** — chỉ sau Enter, hoặc sau `. ! ?` **kèm dấu cách**; không còn `Google.Com.Vn` khi gõ `google.com.vn` hay viết hoa sau `3.14`.
- **Space thừa sau dấu câu** (vd `Xong.  Tiếp`) — vẫn viết hoa đúng.

### ℹ️ Hành vi có chủ đích

- **Không viết hoa ngay sau dấu câu** — domain (`google.com.vn`), số thập phân (`3.14`), viết tắt không space (`Mr.Smith`, `U.S.A.`).
- **Có space sau dấu câu** — `Xong. Tiếp` vẫn viết hoa như bình thường.

### 📝 Giao diện

- Hint **Viết hoa đầu câu** (tab Chung) mô tả đúng: cần dấu cách sau dấu câu.

### 🧪 Tests

- Assert từng segment domain sau dấu chấm; thêm test `!` / `?` không space và `?` + space; test double-space sau dấu chấm. Toàn bộ **281 test pass**.
